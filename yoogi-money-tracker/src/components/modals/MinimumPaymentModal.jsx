    ...
    const handleSubmit = async (e) => {
        ...
        try {
            ...
            const transactionData = {
                ...
                categoryId: isPaying ? 'uncategorized_expense' : 'loan_repaid',
                subcategoryId: '',
            ...
            <div className="bg-white dark:bg-slate-800 rounded-3xl shadow-2xl w-full max-w-md flex flex-col overflow-hidden animate-in fade-in zoom-in-95 duration-200">
                ...
                <div className="px-6 py-4 bg-orange-50 dark:bg-orange-900/20 border-b border-orange-100 dark:border-orange-900/30">
                    <p className="text-sm text-slate-600 dark:text-slate-300">
                        Khoản: <span className="font-bold text-slate-800 dark:text-white">{item.name}</span>
                    </p>
                    <p className="text-sm text-slate-600 dark:text-slate-300">
                        Mức hàng tháng: <span className="font-bold text-orange-500">{formatCurrency(item.monthlyPayment)}</span>
                    </p>
                    <p className="text-xs text-orange-600 dark:text-orange-400 mt-2">
                        Lưu ý: Tiền trả lẻ sẽ được ghi vào Sổ giao dịch, nhưng kỳ trả góp này vẫn sẽ hiển thị là "Chưa trả" cho đến khi bạn bấm xác nhận "Đã trả".
                    </p>
                </div>
                ...
                <form onSubmit={handleSubmit} className="p-6 space-y-4">
                    ...
                    <div className="relative">
                        <span className="absolute top-2 left-4 text-[10px] text-slate-400 font-medium">Chọn ví</span>
                        <select
                            required
                            value={form.walletId}
                            onChange={e => setForm({ ...form, walletId: e.target.value })}
                            className="w-full pl-4 pr-10 pt-6 pb-2 appearance-none border border-slate-200 dark:border-slate-700 bg-white dark:bg-slate-900 text-slate-900 dark:text-white rounded-xl focus:border-orange-500 focus:ring-1 focus:ring-orange-500 focus:outline-none cursor-pointer"
                        >
                    ...
                    <div className="relative">
                        ...
                        <input
                            ...
                            className="w-full px-4 pt-6 pb-2 border border-slate-200 dark:border-slate-700 bg-white dark:bg-slate-900 text-slate-900 dark:text-white rounded-xl focus:border-orange-500 focus:ring-1 focus:ring-orange-500 focus:outline-none"
                        />
                    </div>
                    ...
                    <div className="flex gap-3 pt-2">
                        ...
                        <button
                            type="submit"
                            className="flex-1 py-3.5 bg-orange-500 hover:bg-orange-600 text-white rounded-xl font-bold shadow-lg shadow-orange-500/30 transition-colors"
                        >
                            Xác nhận
                        </button>
                    </div>
...
export default MinimumPaymentModal;
