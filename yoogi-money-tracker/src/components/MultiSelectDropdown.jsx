            ...
            {isOpen && (
                <div className="absolute z-50 min-w-max w-full mt-1 bg-white dark:bg-slate-800 border border-slate-200 dark:border-slate-700 rounded-xl shadow-lg max-h-[65vh] overflow-y-auto">
                    ...
                    {isGrouped ? options.map((group, gIdx) => (
                        <div key={gIdx}>
                            {group.superLabel && (
                                <div className={`px-4 py-2 ${gIdx > 0 ? 'mt-3' : ''} text-sm font-black uppercase tracking-widest border-b border-slate-200 dark:border-slate-700/50 sticky top-0 z-10 bg-white dark:bg-slate-800`}>
                                    {group.superLabel}
                                </div>
                            )}
