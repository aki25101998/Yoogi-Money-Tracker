...
export default function App() {
    ...
    const filteredItems = useMemo(() => {
        ...
        // Disabled logic for now in filtering as per previous request to show split columns
    ...
    // --- Split Items: InProgress (Left) vs Completed (Right) ---
    const { inProgressItems, completedItems } = useMemo(() => {
        ...
        filteredItems.forEach(item => {
            ...
            // Logic: Move to Right if Paid This Month OR Fully Finished
    ...
    const handleSaveItem = async (formData) => {
        ...
        const itemData = {
            ...
            paidMonths: formData.paidMonths || [], // Ensure paidMonths is saved
        };
    ...
    const handleAnalyzeFinances = async () => {
        ...
        try {
            ...
            const response = await fetch(
                `https://generativelanguage.googleapis.com/v1beta/models/gemini-1.5-flash-8b:generateContent?key=${import.meta.env.VITE_GENERATIVE_AI_KEY || ""}`,
                {
                    method: "POST",
            ...
            <header className="bg-white dark:bg-slate-800 border-b border-slate-200 dark:border-slate-700 sticky top-0 z-20 shadow-sm transition-colors duration-200">
                <div className="max-w-6xl mx-auto px-4 py-3">
                    <div className="flex justify-between items-center mb-3 sm:mb-2">
                        ...
                        <div className="flex items-center gap-2">
                            <button onClick={toggleTheme} className="p-2 rounded-full text-slate-500 dark:text-slate-400 hover:bg-slate-100 dark:hover:bg-slate-700 transition-colors mr-1">
                            ...
                            {user && (
                                <div className="flex items-center gap-2 bg-indigo-50 dark:bg-indigo-900/20 px-3 py-1.5 rounded-full border border-indigo-100 dark:border-indigo-800/50">
                                    {user.photoURL ? <img src={user.photoURL} alt="Avatar" className="w-6 h-6 rounded-full" /> : <User className="w-4 h-4 text-indigo-600 dark:text-indigo-400" />}
                                    ...
                                    <button onClick={handleLogout} className="ml-1 text-slate-400 hover:text-rose-500 dark:text-slate-500 dark:hover:text-rose-400" title="Đăng xuất"><LogOut className="w-4 h-4" /></button>
                                </div>
                            )}
                        </div>
                    </div>
                    ...
                    {user && (
                        <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-3 pt-2 border-t border-slate-100 dark:border-slate-700">
                            ...
                            <div className="flex gap-2 ml-auto items-center">
                                <button onClick={() => setHideCompleted(!hideCompleted)} className={`flex items-center gap-1.5 px-3 py-1.5 rounded-lg text-xs font-medium transition-all border select-none ${hideCompleted ? 'bg-indigo-600 text-white border-indigo-600 shadow-sm shadow-indigo-200 dark:shadow-none' : 'bg-white dark:bg-slate-800 text-slate-600 dark:text-slate-300 border-slate-200 dark:border-slate-600 hover:bg-slate-50 dark:hover:bg-slate-700'}`}>
                                    <div className={`w-3.5 h-3.5 rounded flex items-center justify-center transition-colors ${hideCompleted ? 'bg-white text-indigo-600' : 'bg-slate-200 dark:bg-slate-600'}`}>{hideCompleted && <Check className="w-2.5 h-2.5" strokeWidth={3} />}</div>
                ...
                {!user ? (
                    <div className="flex flex-col items-center justify-center py-20 bg-white dark:bg-slate-800 rounded-2xl shadow-sm border border-slate-100 dark:border-slate-700 text-center p-6 transition-colors">
                        <div className="bg-indigo-100 dark:bg-indigo-900/30 p-4 rounded-full mb-6 relative">
                            ...
                            <div className="absolute -bottom-1 -right-1 bg-green-500 rounded-full p-1 border-2 border-white dark:border-slate-800"><Sparkles className="w-3 h-3 text-white" /></div>
                        </div>
                        <h2 className="text-2xl font-bold text-slate-800 dark:text-white mb-3">Chào mừng trở lại!</h2>
                        <p className="text-slate-500 dark:text-slate-400 max-w-sm mb-8">Đăng nhập để quản lý các khoản trả góp của bạn.</p>
                        <div className="space-y-3 w-full max-w-xs">
                            <button onClick={handleGoogleLogin} className="w-full flex items-center justify-center gap-3 px-6 py-3.5 bg-indigo-600 hover:bg-indigo-700 text-white rounded-xl font-bold shadow-lg shadow-indigo-200 dark:shadow-none transition-all hover:-translate-y-0.5"><LogIn className="w-5 h-5" /> Đăng nhập với Google</button>
                            {!auth && <p className="text-xs text-rose-500 bg-rose-50 dark:bg-rose-900/20 p-2 rounded-lg border border-rose-100 dark:border-rose-900/30">⚠️ Lỗi cấu hình Firebase. Vui lòng kiểm tra file .env</p>}
                        </div>
                    </div>
                ) : (
                    <>
                        {isLoading ? (
                            <div className="flex flex-col items-center justify-center py-20 text-slate-400"><Loader2 className="w-10 h-10 animate-spin mb-3 text-indigo-500" /><p>Đang tải dữ liệu từ Cloud...</p></div>
                        ) : (
                            <>
